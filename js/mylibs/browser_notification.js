...
// Function to check if this Cookie exists
...
// Function to set cookie, if needed
function setCookie() {
	cookie_made = 'true';
	// If no cookie "browser_validation_notified" found
...
// Function to detect baseline features and whether combatibility mode is turned on, then notify browser in event of failure
function browserDetectNotify() {
	...
	// Run Modernizr first, because IE9 can contribute false positives on feature support when in compat. mode
	...
	if ( (Modernizr.fontface && Modernizr.canvas ) ) { 
	    // script to run if browser passes feature test
	} else {
	...
	// Written by Leo Liu [MSFT]
	// Found on: http://social.msdn.microsoft.com/Forums/en/netfxjscript/thread/ae715fd2-1ddd-46f7-8c26-9aed6b2103f1
	// If browser user-agent registers as less than IE 9
	if ( ($.browser.msie) && ($.browser.version < '9.0') ){
		
		//script to extract compat. mode string
		var agentStr = navigator.userAgent;
        var mode;

        //Add additional browser modes to this when new IE versions are released, if neccessary.
		...
		// End Leo Liu [MSFT] IE version/compat checker

		// Output compat mode value (if present). Add additional browser modes to this when new IE versions are released, if neccessary.
		if( ( mode == 'IE9 Compatibility View') || ( mode == 'IE8 Compatibility View' ) ) {
			compat = 'true';
		}
		else{
		}
	...
	if ( legacy != '' || compat != '' ) {
		...
		if (compat == 'true') {
			//console.log('legacy = compat, launching compat message');
			$('#compat_message').css('display', 'block');

		} else if (legacy == 'true'){
			//console.log('legacy = true, launching legacy message');
